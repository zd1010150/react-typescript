...
import { createNewAccount } from '../../auth/createAccount/flow/actions';
import { ICreateAccountForm } from '../../auth/createAccount/flow/types';
...
export class EditAccount extends React.Component <IcreateAccountProps, {}> {
    ...
    private createNewAccount = async() => {
        ...
        Promise.all([
            this.companyInfoForm.validateResult, 
            this.contactPersonForm.validateResult, 
            this.signInForm.validateResult, 
            this.passwordForm.validateResult
        ]).then(([companyInfoForm,contactPersonForm, signInfoForm, passwordForm])=>{
            const validateResult = companyInfoForm.validate && contactPersonForm.validate && signInfoForm.validate && passwordForm.validate;
            if(validateResult){
                const postData: ICreateAccountForm = {
